  ...
  return (
    <div className="max-w-7xl mx-auto space-y-6">
            ...
            <div>
              ...
              <div className="grid grid-cols-2 gap-2">
                {(['global', 'department', 'section', 'team'] as const).map((level) => (
                  <button key={level} onClick={() => setViewLevel(level)} className={`p-3 text-sm rounded-lg border transition-all capitalize ${ viewLevel === level ? 'bg-gradient-hero text-white border-transparent' : 'bg-white text-gray-700 border-gray-300 hover:bg-gray-50'}`}>{level === 'global' ? 'All Organization' : level}</button>
                ))}
              </div>
              <div className="mt-4">
      ...
      <div className="text-center p-4 bg-white/50 rounded-xl">
        <h2 className="text-xl font-semibold text-gray-900">{getLevelTitle()}</h2>
        <p className="text-gray-600 mt-1">{leaderboard.length} participants competing</p>
      </div>
      ...
      <div className="grid grid-cols-1 md:grid-cols-3 gap-6">
        <Card hover className="bg-blue-50">
          <div className="flex items-center p-6">
            <div className="p-3 rounded-lg bg-blue-100 mr-4"><Users className="h-6 w-6 text-blue-600" /></div>
            <div>
        ...
        <Card hover className="bg-green-50">
          <div className="flex items-center p-6">
            <div className="p-3 rounded-lg bg-green-100 mr-4"><Target className="h-6 w-6 text-green-600" /></div>
            <div>
              <p className="text-sm font-medium text-gray-600">Total Problems</p>
              <p className="text-2xl font-bold text-gray-900">{leaderboard.reduce((sum, entry) => sum + entry.totalSolved, 0).toLocaleString()}</p>
            </div>
          </div>
        </Card>
