...
const LeaderboardTable: React.FC<LeaderboardTableProps> = ({ data, showTeamColumn = true }) => {
  ...
  const getRankBadge = (rank: number) => {
    if (rank <= 3) {
      const colors = { 1: 'bg-yellow-100 text-yellow-800', 2: 'bg-gray-100 text-gray-800', 3: 'bg-amber-100 text-amber-800' } as const;
            ...
            <tr key={`${entry.memberId}-${entry.rank}`} className={`hover:bg-gray-50 ${entry.rank <= 3 ? 'bg-gradient-to-r from-gray-50 to-white' : ''} ${entry.isTeamLead ? 'bg-yellow-50/30' : ''}`}>
              <td className="px-6 py-4 whitespace-nowrap">
                <div className="flex items-center space-x-2">
                  <span className={`inline-flex items-center justify-center w-8 h-8 rounded-full text-sm font-bold ${getRankBadge(entry.rank)}`}>#{entry.rank}</span>
                ...
                <div className="flex items-center">
                  <div>
                    <div className="flex items-center space-x-2">
                      ...
                      {entry.isTeamLead && (
                        <span className="inline-flex items-center bg-yellow-100 text-yellow-800 px-2 py-0.5 rounded-full text-xs font-medium">👑 Team Lead</span>
                      )}
                    </div>
              ...
              <td className="px-6 py-4 whitespace-nowrap text-center"><span className="text-sm font-medium text-orange-600">{entry.leetcodeTotal}</span></td>
              <td className="px-6 py-4 whitespace-nowrap text-center"><span className="text-sm font-medium text-teal-600">{entry.skillrackTotal}</span></td>
              <td className="px-6 py-4 whitespace-nowrap text-center"><span className="text-sm font-medium text-amber-700">{entry.codechefTotal}</span></td>
              <td className="px-6 py-4 whitespace-nowrap text-center"><span className="text-sm font-medium text-emerald-600">{entry.hackerrankTotal}</span></td>
              <td className="px-6 py-4 whitespace-nowrap text-center"><div className={`text-lg font-bold ${entry.rank <= 3 ? 'text-yellow-600' : entry.isTeamLead ? 'text-yellow-700' : 'text-gray-900'}`}>{entry.totalSolved.toLocaleString()}</div></td>
            </tr>
          ))}
        </tbody>
      </table>
